...
from neptune_api_v2 import NeptuneAPIV2, AsyncNeptuneAPIV2, DefaultAioHttpClient
from neptune_api_v2._utils import is_dict
...
logging.getLogger("neptune_api_v2").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[NeptuneAPIV2]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with NeptuneAPIV2(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncNeptuneAPIV2]:
